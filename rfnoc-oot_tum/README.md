...
* `examples`: Any example that shows how to use this OOT module goes here.
  Examples that are listed in CMake get installed into `share/rfnoc-oot_tum/examples`
  within the installation prefix.

* `rfnoc/fpga/oot_tum`: This directory contains the gateware for the HDL modules
  ...
  module name ('oot_tum') is included in the path. When installing this OOT
  module, these files get copied to the installation path's RFNoC package dir
  (e.g., `/usr/share/uhd/rfnoc/fpga/oot_tum`) so the image builder can find
  all the source files when compiling bitfiles that use multiple OOT modules.

* `gr-rfnoc_oot_tum`: An out-of-tree module for GNU Radio, which depends on this
  RFNoC out-of-tree module and provides GNU Radio and GNU Radio Companion
  bindings for using the oot_tum block from within GNU Radio.
...
* `icores`: Stores full image core files. YAML files in this directory get
  ...
  an X310 image core with the oot_tum block included. You can build this image
...
* `include/rfnoc/oot_tum`: Here, all the header files for the block controllers
  ...
  e.g., these header files could get installed to `/usr/include/rfnoc/oot_tum`.
  By mirroring the path name, we can write
  `#include <rfnoc/oot_tum/oot_tum_block_control.hpp>` in our C++ source code, and
...
directory structure in this example, e.g., `/usr/share/uhd/rfnoc/fpga/oot_tum` will
have the same content as `./fpga/oot_tum`. This allows the image builder to work
...
```sh
cd /path/to/rfnoc-oot_tum/build
...
```sh
cd /path/to/rfnoc-oot_tum  # Switch to this file's directory
