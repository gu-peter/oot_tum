...
graph_connections = {
    0: {"tx": ("0/Radio#0", 2), "rx": ("0/Radio#0", 2)},
    1: {"tx": ("0/Radio#0", 1), "rx": ("0/Radio#0", 1)},
    2: {"tx": ("0/Radio#1", 0), "rx": ("0/Radio#1", 0)},
    3: {"tx": ("0/Radio#1", 1), "rx": ("0/Radio#1", 1)},
}
...
def main():
    ...
    # Create a CW tone per channel
    t = np.arange(NUM_SAMPLES) / NUM_SAMPLES
    symbols_in = np.zeros((num_chan, NUM_SAMPLES), dtype=np.complex128)
    for chan_idx in range(num_chan):
        symbols_in[chan_idx] = AMPLITUDE * np.exp(2j * np.pi * t)
    ...
    for chan_idx, chan in enumerate(CHANNELS):
        ...
        axes[0][chan_idx].plot(np.real(symbols_in[chan_idx]), label="I")
        axes[0][chan_idx].plot(np.imag(symbols_in[chan_idx]), label="Q")
        axes[0][chan_idx].legend()
        axes[0][chan_idx].grid(True)
        axes[1][chan_idx].set_title(f"Channel {chan} RX")
        axes[1][chan_idx].plot(np.real(symbols_out[chan_idx]), label="I")
        axes[1][chan_idx].plot(np.imag(symbols_out[chan_idx]), label="Q")
